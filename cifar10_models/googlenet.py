...
def googlenet(pretrained=False, target=-1, progress=True, device='cpu', **kwargs):
    ...
    if pretrained:
        ...
        if target >= 0:
            state_dict = torch.load('{}/state_dicts/googlenet/{}.pt'.format(script_dir, target), map_location=device)
        else:
